...
(function (EnqueuePro) {
            ...
            EnvironmentHelper.retrieveEnvInfoFromScriptTag = function () {
                ...
                try {
                    ...
                    for (var i = 0; i < scripts.length; i++) {
                        ...
                        var cid = scriptTag.getAttribute("data-enqueuepro-c");
                        if (cid) {
                            var host = scriptTag.getAttribute("data-enqueuepro-host") || "";
                            var interceptAttr = scriptTag.getAttribute("data-enqueuepro-intercept") || "";
                            var intercept = interceptAttr.toLowerCase() == "true";
                            var domainToIntercept = scriptTag.getAttribute("data-enqueuepro-intercept-domain") || "";
                            var jsHost = scriptTag.getAttribute("data-enqueuepro-jshost") || "";
                            var noCacheRAttr = scriptTag.getAttribute("data-enqueuepro-nocachereq") || "";
                            var noCacheR = noCacheRAttr.toLowerCase() == 'true';
                            var noAutorunAttr = scriptTag.getAttribute("data-enqueuepro-noautorun") || "";
                            var noAutorun = noAutorunAttr.toLowerCase() == 'true';
                            var enqueueproKURedirectHeaderName = scriptTag.getAttribute("data-enqueuepro-ku-redirect-headername") || "";
                            EnvironmentHelper.environemntVariable = {
                                ...
                                enqueueproKURedirectHeaderName: enqueueproKURedirectHeaderName
                            };
                            return EnvironmentHelper.environemntVariable;
                        }
                    }
    ...
    })(Tools = EnqueuePro.Tools || (EnqueuePro.Tools = {}));
})(EnqueuePro || (EnqueuePro = {}));
        ...
        (function (KnownUser) {
            var AjaxInterceptor = /** @class */ (function () {
                ...
                AjaxInterceptor.interceptOpen = function (domainToIntercept, en, getCurrentUrl, redirector) {
                    ...
                    var enqueueproKURedirectHeaderName = "x-enqueuepro-redirect";
                    if (en && en.enqueueproKURedirectHeaderName) {
                        enqueueproKURedirectHeaderName = en.enqueueproKURedirectHeaderName;
                    }
                    if (!redirector) {
                    ...
                    if ((typeof XMLHttpRequest === 'function' || typeof XMLHttpRequest === 'object' /*old safari and test engine for unitest*/)
                        ...
                        var stateIsUpdated = function (xhr) {
                            if (xhr.readyState === XMLHttpRequest.DONE) {
                                try {
                                    var queueUrl = xhr.getResponseHeader(enqueueproKURedirectHeaderName);
                                    if (queueUrl) {
                                        redirector(decodeURIComponent(queueUrl));
                                    }
                                }
                                catch (ex) {
                                    console.log("getResponseHeader('" + enqueueproKURedirectHeaderName + "') error" + ex);
                                }
                            }
                        };
                        XMLHttpRequest.prototype.open = function (method, url, async, user, password) {
                            ...
                            try {
                                ...
                                if (addQueueITHeaders) {
                                    xhr.setRequestHeader("x-enqueuepro-ajaxpageurl", encodeURIComponent(getCurrentUrl()));
                                }
                            }
                        };
                    }
        ...
        (function (KnownUser) {
            var UrlQueueClientInitializer = /** @class */ (function () {
                ...
                UrlQueueClientInitializer.tryInitQueueClientFromToken = function (en, url) {
                    ...
                    if (!token ||
                        !token.eventId || !EnqueuePro.Javascript.QueueClient || window.myQueueClient)
                        return;
                ...
                UrlQueueClientInitializer.getQueueITToken = function (currentUrl) {
                    var regex = /enqueuetoken=([^&]*)/i;
                    var match = regex.exec(currentUrl);
                    if (!match || !match[1])
                        return null;
                    var result = new EnqueueProToken();
                    result.rawEnqueueProToken = match[1];
                    var tokenParts = result.rawEnqueueProToken.split("~");
            ...
            var EnqueueProToken = /** @class */ (function () {
                function EnqueueProToken() {
                }
                return EnqueueProToken;
            }());
            KnownUser.EnqueueProToken = EnqueueProToken;
        })(KnownUser = Javascript.KnownUser || (Javascript.KnownUser = {}));
    })(Javascript = EnqueuePro.Javascript || (EnqueuePro.Javascript = {}));
})(EnqueuePro || (EnqueuePro = {}));
    ...
    (function (Javascript) {
        ...
        (function (IntegrationConfig) {
            ...
            function applyIntegrationConfig(config, integrationPointFactory, getCurrentProtocol) {
                ...
                for (var _i = 0, _a = config.integrations; _i < _a.length; _i++) {
                    ...
                    if (integraionPoint.evaluate()) {
                        if (integraionPoint.getActionType() === IntegrationConfig.Model.ActionType.QueueAction) {
                            ...
                            var cookieManager = new EnqueuePro.Javascript.CookieManager(config.customerId, integration.getEventId(), getCurrentProtocol() + '//' + queueDomain, integration.getCookieDomain());
                            cookieManager.cancelQueueId();
                        }
                        else if (integraionPoint.getActionType() === IntegrationConfig.Model.ActionType.IgnoreAction) {
                            var en_1 = EnqueuePro.Tools.EnvironmentHelper.retrieveEnvInfoFromScriptTag();
                            if (en_1.intercept)
                                EnqueuePro.Javascript.KnownUser.UrlQueueClientInitializer.tryInitQueueClientFromToken(en_1);
                        }
                        return;
                    }
                }
                ...
                if (en.intercept)
                    EnqueuePro.Javascript.KnownUser.UrlQueueClientInitializer.tryInitQueueClientFromToken(en);
            }
            IntegrationConfig.applyIntegrationConfig = applyIntegrationConfig;
            //backward compatibility (javascript created by queuefront)
            EnqueuePro.Javascript.PageEventIntegration = { initQueueClient: applyIntegrationConfig };
            ...
            function isIntegrationConfigReady() {
                if (window.enqueuepro_clientside_config)
                    return true;
                return false;
            }
            function getIntegrationConfig() {
                return window.enqueuepro_clientside_config;
            }
            EnqueuePro.getIntegrationConfig = getIntegrationConfig;
            EnqueuePro.isIntegrationConfigReady = isIntegrationConfigReady;
        })(IntegrationConfig = Javascript.IntegrationConfig || (Javascript.IntegrationConfig = {}));
    })(Javascript = EnqueuePro.Javascript || (EnqueuePro.Javascript = {}));
})(EnqueuePro || (EnqueuePro = {}));